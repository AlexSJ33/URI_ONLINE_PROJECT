"""
    ...
    uma casa decimal, correspondente às quatro notas de um aluno.
    ...
    inferior a 5.0, imprima a mensagem "Aluno reprovado.".
    ...
    linha uma mensagem "Media final: " seguido da média
    final para esse aluno.
    Entrada
    A entrada contém quatro números de ponto flutuante
    correspendentes as notas dos alunos.
    Saída
    Todas as respostas devem ser apresentadas com 
    uma casa decimal. As mensagens devem ser impressas
    conforme a descrição do problema. Não esqueça de imprimir
    o enter após o final de cada linha, caso contrário obterá
    "Presentation Error".
"""
...
if mp >= 7.0:
    ...
    x = float(input('Nota do exame: '))
    mf = (x+mp)/2
    if mf >= 5.0:
        print('Aluno aprovado.')
        print('Média final: %0.1f'%mf)
    else:
        print('Aluno reprovado.')
        print('Média final: %0.1f'%mf)